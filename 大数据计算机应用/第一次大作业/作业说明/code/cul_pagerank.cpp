...
{
  TRACK_FUNCTION_BEGIN("build_size");
  ...
  TRACK_FUNCTION_END("build_size");
  TRACK_FUNCTION_BEGIN("build_node_index");
  ...
  TRACK_FUNCTION_END("build_node_index");
  TRACK_FUNCTION_BEGIN("build_triplets");
  // 重置文件指针到开头
  file.clear();
  file.seekg(0, std::ios::beg);
  // 建立out_degrees初度
  ...
  TRACK_FUNCTION_END("build_triplets");
...
int main()
{
  ...
  // 将Top-100节点写入文件
  TRACK_FUNCTION_BEGIN("w");
  std::ofstream outfile("Res8_cul.txt");
  if (outfile.is_open())
  {
