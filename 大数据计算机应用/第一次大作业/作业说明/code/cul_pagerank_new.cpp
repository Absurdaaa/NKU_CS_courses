...
{
  TRACK_FUNCTION_BEGIN("build_node");
                      ...
                      int idx_end)
{
  ...
  TRACK_FUNCTION_END("getline");
  TRACK_FUNCTION_BEGIN("build_graph_part");
  file.clear();
  file.seekg(0, std::ios::beg);
  ...
  TRACK_FUNCTION_END("build_graph_part");
}
...
void build_death(
    ...
    std::vector<int> &out_degrees)
{
  TRACK_FUNCTION_BEGIN("build_death");
  int N = nodes.size();
  death.resize(N, 0.0f);
  for (int i = 0; i < N; i++)
  {
  ...
  TRACK_FUNCTION_END("build_death");
}
                            ...
                            float damping = 0.85f, float tol = 1e-6f, int max_iter = 10000)
{
  ...
  for (int iter_count = 0; iter_count < max_iter; iter_count++)
  {
    ...
    for (int part = 0; part < parts_num; part++)
    {
      ...
      std::vector<float> pr_new_part = VectorOps::multiply(M_part.multiply(pr), damping);
      // 逐元素加到pr_new
...
int main()
{
  ...
  // 将Top-100节点写入文件
  std::ofstream outfile("Res922.txt");
  if (outfile.is_open())
  {
