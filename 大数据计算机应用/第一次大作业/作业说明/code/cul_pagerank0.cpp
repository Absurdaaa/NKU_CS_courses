...
private:
  ...
  static std::map<std::string, std::vector<size_t>> functionCalls;
...
public:
  ...
  // 函数进入时调用
  static void functionEnter(const std::string &functionName)
  {
    size_t memBefore = getCurrentMemoryUsage();
    functionCalls[functionName].push_back(memBefore);
  }

  // 函数退出时调用
  static void functionExit(const std::string &functionName)
  {
    size_t memAfter = getCurrentMemoryUsage();
    size_t memBefore = functionCalls[functionName].back();
    functionCalls[functionName].pop_back();

    // 累计内存变化
    memoryUsage[functionName] += (memAfter > memBefore) ? (memAfter - memBefore) : 0;
  }
  ...
  {
    std::cout << "\n====== 函数内存使用统计 ======\n";
    std::cout << std::left << std::setw(30) << "函数名"
              << std::right << std::setw(15) << "内存使用(MB)" << std::endl;
    ...
    for (const auto &entry : memoryUsage)
    {
      std::cout << std::left << std::setw(30) << entry.first
                ...
                << (entry.second / (1024.0 * 1024.0)) << std::endl;
    }
    std::cout << std::string(45, '-') << std::endl;
  }
};
...
// 初始化静态成员
std::map<std::string, size_t> MemoryTracker::memoryUsage;
std::map<std::string, std::vector<size_t>> MemoryTracker::functionCalls;

// 方便使用的宏
#define TRACK_FUNCTION_BEGIN(name) MemoryTracker::functionEnter(name)
#define TRACK_FUNCTION_END(name) MemoryTracker::functionExit(name)
...
{
  TRACK_FUNCTION_BEGIN("build_graph");
  ...
  TRACK_FUNCTION_END("build_graph");
}

// 构建转移矩阵和死节点向量
void build_transition_matrix(std::vector<std::pair<std::string, std::string> > &edges,
                             int &edges_size,
                             ...
                             std::vector<int> &out_degrees)
{
  TRACK_FUNCTION_BEGIN("build_transition_matrix");
  ...
  {
    int i_idx = node_index.at(edges[i+1].first);
    int j_idx = node_index.at(edges[i+1].second);
  ...
  TRACK_FUNCTION_END("build_transition_matrix");
}
...
int main()
{
  ...
  // 构建图
  build_graph(file_path, edges, edges_size, nodes, node_index, out_degrees);
  std::cout << "图中有 " << nodes.size() << " 个节点和 " << edges_size << " 条边" << std::endl;

  // 构建转移矩阵
  SparseMatrix M;
  std::vector<float> death;
  build_transition_matrix(edges, edges_size, nodes, node_index, M, death, out_degrees);
  ...
  // 将Top-100节点写入文件
  std::ofstream outfile("Res13.txt");
  if (outfile.is_open())
  {
    for (int i = 0; i < (int)node_pr.size(); i++)
    {
      outfile << node_pr[i].first << " " << std::fixed << std::setprecision(8)
