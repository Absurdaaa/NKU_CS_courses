...
class KNNRecommender:
    ...
    """
    def __init__(self, k=50, method='item', similarity='pearson_baseline'):
        """
        初始化KNN推荐器
            ...
            for i in range(n_rows):
                ...
                for j in range(i, n_rows):
                    ...
                    if np.sum(mask) > 1:  # 至少需要两个共同评分
                        ...
                        b = matrix[j][mask]
                        # 计算基线预测
                        baseline_i = self.user_means[i] if self.method == 'user' else self.item_means[i]
                        baseline_j = self.user_means[j] if self.method == 'user' else self.item_means[j]
                        # 计算皮尔逊相关系数
                        a_centered = a - baseline_i
                        b_centered = b - baseline_j
                        # 如果标准差为0，直接设为0
                        if np.std(a_centered) == 0 or np.std(b_centered) == 0:
                            similarity = 0
                        else:
    ...
    def eval(self, test_df, output_file="prediction_results.txt"):
        """
        在测试集上评估模型性能
        """
        print("在测试集上评估模型...")
...
def optimize_knn_parameters(train_df, test_df):
    ...
    # 定义要测试的参数
    methods = [ 'item']
    similarities = ['pearson','pearson_baseline']
...
if __name__ == "__main__":
    ...
    if do_optimize:
        ...
        similarity = 'pearson_baseline'  # 'cosine' or 'pearson'
        k = 11  # 邻居数量
...
'''
训练KNN模型 (方法: item, 相似度: pearson_baseline, k: 11)...
计算相似度矩阵...
训练完成，耗时: 539.97秒
在测试集上评估模型...
测试集评估完成，耗时: 2.50秒
