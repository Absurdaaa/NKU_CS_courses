...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear((opt.n_classes + int(np.prod(img_shape))), 512), 
                                   ...
                                   nn.Linear(512, 1),
                                  #  nn.Sigmoid()  # 新增Sigmoid，输出[0,1]匹配MSELoss
                                   )
...
# 加载MNIST（关键：指定本地路径+禁用下载）
mnist_path = "./datasets/MNIST/"  # 服务器上4个文件的目录
dataloader = datasets.MNIST(
    train=True,
    transform=transform,
    data_root=mnist_path,  # 指向存放4个.gz文件的目录
    download=False  # 强制禁用自动下载，避免尝试联网
).set_attrs(batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)  # 对应你的opt参数
