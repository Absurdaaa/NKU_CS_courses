...
{
//  如果新结点是 LeafNode
this->child_num.insert(this->child_num.begin() + index, ((LeafNode *)new_node)->cipher.size()); ((LeafNode *)new_node)->parent = this;
}
else
{
//  如果是 InternalNode
...
// 插入的新 node 改变了原有 node 位置，因此需要重新记录 parent_index 和 child_num
...
if (debug_enabled) {
fprintf(stderr, "[FHOPE-DEBUG] InternalNode rebalance, children=%zu\n", this->child.size());
}
rebalance_cnt++;
InternalNode *new_node = new InternalNode();
// 将当前结点的后半部分数据存入 new_node
int middle = floor(this->child.size() * 0.5);
while (middle > 0)
{
new_node->child.insert(new_node->child.begin(), this->child.at(this->child.size() - 1));
new_node->child_num.insert(new_node->child_num.begin(), this->child_num.at(this->child_num.size() - 1)); this->child.pop_back();
...
if (debug_enabled) {
int total = 0;
for (size_t i = 0; i < node_list.size(); i++) total += node_list.at(i)->cipher.size();
fprintf(stderr, "[FHOPE-DEBUG] Recode: %zu nodes, %d ciphers, interval=[%lld, %lld)\n",
node_list.size(), total, node_list.at(0)->lower, node_list.back()->upper);
}
recode_cnt++;
...
{
//  如果当前的更新区间，足以包含待放的 pos
start_update = left_bound;
end_update = right_bound;
//  计算间隔量，使 code 均匀分布
long long frag = floor((right_bound - left_bound) / total_cipher_num); assert(frag >= 1);
...
update.insert(make_pair(node_list.at(i)->cipher.at(j), cd)); cd = cd + frag;
}
...
if (node_list.back()->upper >= pow(2, 9)) node_list.back()->upper = pow(2, 9);
}
Recode(node_list); }
}
...
{
//  如果该区间已经没有位置，则需要 recode
std::vector<LeafNode *> node_list;
node_list.push_back(this);
Recode(node_list);
//  返回特殊值 0，标志已经调整，需要更新数据库
return 0; }
else
{
//  否则直接以 left 和 right 的平均值向上取整作为新的 code
...
if (debug_enabled) {
fprintf(stderr, "[FHOPE-DEBUG] LeafNode rebalance, ciphers=%zu total_global=%d\n",
this->cipher.size(), rebalance_cnt);
}
rebalance_cnt++;
LeafNode *new_node = new LeafNode();
// 将本结点后半部分数据放到 new_node
int middle = floor(this->cipher.size() * 0.5);
while (middle > 0)
{
new_node->cipher.insert(new_node->cipher.begin(), this->cipher.back());
new_node->encoding.insert(new_node->encoding.begin(), this->encoding.back()); this->encoding.pop_back();
...
if (!this->parent)
{
//  如果 this 是 root 结点，则创建一个新的 root
InternalNode *new_root = new InternalNode();
new_root->insert_node(0, this);
new_root->insert_node(1, new_node);
root = new_root;
...
{
((InternalNode *)this->parent)->child_num.at(this->parent_index) = this->cipher.size(); ((InternalNode *)this->parent)->insert_node(this->parent_index + 1, new_node);
}
}
