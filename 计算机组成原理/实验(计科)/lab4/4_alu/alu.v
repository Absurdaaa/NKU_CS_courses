...
module alu(
    ...
    always @(*) begin
    case (alu_control2)
        4'b0000: alu_control = 15'b0000_0000_0000_0001;
        4'b0001: alu_control = 15'b0000_0000_0000_0010;
        4'b0010: alu_control = 15'b0000_0000_0000_0100;
        4'b0011: alu_control = 15'b0000_0000_0000_1000;
        4'b0100: alu_control = 15'b0000_0000_0001_0000;
        4'b0101: alu_control = 15'b0000_0000_0010_0000;
        4'b0110: alu_control = 15'b0000_0000_0100_0000;
        4'b0111: alu_control = 15'b0000_0000_1000_0000;
        4'b1000: alu_control = 15'b0000_0001_0000_0000;
        4'b1001: alu_control = 15'b0000_0010_0000_0000;
        4'b1010: alu_control = 15'b0000_0100_0000_0000;
        4'b1011: alu_control = 15'b0000_1000_0000_0000;
        4'b1100: alu_control = 15'b0001_0000_0000_0000;
        4'b1101: alu_control = 15'b0010_0000_0000_0000;
        4'b1110: alu_control = 15'b0100_0000_0000_0000;
        4'b1111: alu_control = 15'b1000_0000_0000_0000;
        default: alu_control = 15'b0000_1000_0000_0000;
    endcase
end

    // ALU控制信号，独热码
    wire alu_and_not;   //按位与非
    wire alu_rem;   //  a%b
    wire alu_big;   //有符号比较，大于置位，复用加法器做减法
    wire alu_bigu;  //无符号比较，大于置位，复用加法器做减法
    ...
    assign alu_and_not  = alu_control[15];
    assign alu_rem  = alu_control[14];
    assign alu_big  = alu_control[13];
    assign alu_bigu = alu_control[12];
    ...
    wire [31:0] big_result;
    wire [31:0] bigu_result;
    ...
    assign and_not_result = ~(alu_src1 & alu_src2);      // 与结果为两数按位与非
    assign rem_result  = alu_src1 % alu_src2;      // 取模
...
//add,sub,slt,sltu均使用该模块
    ...
    //判断两个是否相等
    wire equ = adder_result == 32'd0 ?1:0;
    assign big_result[31:1] = 31'd0;
                               //正-负 | （~负-正 & ~相等 &~）
    assign big_result[0]    = (~alu_src1[31] & alu_src2[31]) | (~(alu_src1[31]^alu_src2[31]) & ~equ & ~adder_result[31]);
    ...
    //对比slt结果注释，知道，此时判断大小属于第二三种情况，即源操作数1符号位为0，源操作数2符号位为0
    //结果的符号位为1，说明小于，即adder_cout+1'b1为2'b01，即adder_cout为0
    wire bigger = adder_cout & (~equ);
    assign bigu_result = {31'd0, adder_cout & (~equ)};
//-----{加法器}end
