...
class SlidingWindowTests(unittest.TestCase):
    ...
    def setUp(self):
        """创建各测试共用的待测接口。"""
        self.api = self._api()
        self.bruteforce_api = self._bruteforce_api()
    ...
    def test_bruteforce_api_returns_expected_values_for_standard_case(self):
        ...
        result = self.bruteforce_api(nums, 3)
    ...
    def test_bruteforce_and_optimized_implementations_match(self):
        ...
        optimized_result = self.api(nums, 4)
        bruteforce_result = self.bruteforce_api(nums, 4)
