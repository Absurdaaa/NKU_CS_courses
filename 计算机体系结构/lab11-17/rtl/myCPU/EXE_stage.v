...
module exe_stage(
    ...
    output [18:0]                   tlb_s1_vpn2,
    output                          tlb_s1_odd_page,
    output [7:0]                    tlb_s1_asid
);
...
assign {
    ...
    es_inst_tlbp   ,  //157:157
    es_inst_tlbwi  ,  //156:156
    es_inst_tlbr   ,  //155:155
    es_inst_lb     ,  //154:154
    es_inst_lbu    ,  //153:153
    es_inst_lh     ,  //152:152
    es_inst_lhu    ,  //151:151
...
wire [31:0] es_data_vaddr;
assign es_data_vaddr = (es_inst_lwl || es_inst_swl) ? {es_alu_result[31:2], 2'b0} : es_alu_result;
...
assign es_to_ms_bus = {
    es_tlbp_found  ,  //170:170
    es_tlbp_index  ,  //169:166
    es_inst_tlbp   ,  //165:165
    es_inst_tlbwi  ,  //164:164
    es_inst_tlbr   ,  //163:163
    es_data_ok      ,  //162:162
    es_data         ,  //161:130
    es_excode       ,  //129:125
    es_badvaddr     ,  //124:93
...
always @ (posedge clk) begin
    if (reset) begin
        ...
        es_addr_ok_r <= 1'b1;
    end else if (ms_allowin || ws_eret || ws_ex || ws_tlb_flush) begin
        es_addr_ok_r <= 1'b0;
    end
end
...
always @ (posedge clk) begin
    if (reset) begin
        ...
        es_data_buff        <= 32'h0;
    end else if (ms_allowin || no_store || ws_tlb_flush) begin
        es_data_buff_valid  <= 1'b0;
        es_data_buff        <= 32'h0;
...
assign es_to_ms_valid =  es_valid && es_ready_go && !ws_eret && !ws_ex && !ws_tlb_flush;
always @(posedge clk) begin
    if (reset) begin
        es_valid <= 1'b0;
