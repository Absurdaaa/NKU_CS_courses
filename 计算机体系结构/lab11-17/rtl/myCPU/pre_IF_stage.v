...
assign pfs_to_fs_valid  = pfs_valid && pfs_ready_go && !ws_eret && !ws_ex && !ws_tlb_flush;
...
always @ (posedge clk) begin
    if (reset) begin
        br_taken_r  <= 1'b0;
        br_target_r <= 32'h0;
    end else if (target_leaving_pfs || ws_eret || ws_ex || ws_tlb_flush) begin
        br_taken_r  <= 1'b0;
        br_target_r <= 32'h0;
...
always @ (posedge clk) begin
    if (reset) begin
        ...
        seq_pc <= cp0_epc;
    end else if (ws_tlb_flush) begin
        seq_pc <= ws_tlb_flush_pc;
...
always @ (posedge clk) begin
    if (reset) begin
        ...
        pfs_addr_ok_r <= 1'b1;
    end else if (fs_allowin || ws_eret || ws_ex || ws_tlb_flush) begin
        pfs_addr_ok_r <= 1'b0;
    end
end
