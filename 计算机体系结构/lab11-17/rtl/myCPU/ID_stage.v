...
module id_stage(
    ...
    // tlbp hazard with mtc0 entryhi (no forwarding, stall)
    input                           es_mtc0_entryhi,
...
assign ds_ready_go    = !(
    mfc0_block || tlbp_entryhi_block ||
    (es_blk_valid  && (es_rf_dest == rs || es_rf_dest == rt)) ||
    (ms_blk_valid && (ms_rf_dest == rs || ms_rf_dest == rt))
);

assign ds_allowin     = !ds_valid || ds_ready_go && es_allowin;
assign ds_to_es_valid = ds_valid && ds_ready_go && !ws_eret && !ws_ex && !ws_tlb_flush;
...
always @ (posedge clk) begin
    if (reset) begin
        ds_is_bd_r <= 1'b0;
    end else if (ws_eret || ws_ex || ws_tlb_flush) begin
        ds_is_bd_r <= 1'b0;
