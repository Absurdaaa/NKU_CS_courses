...
torch::Tensor hsigmoid_cpu(const torch::Tensor & dets) {
  ...
  auto tensor_contiguous = dets.contiguous();
  std::vector<float> input_data(tensor_contiguous.data_ptr<float>(), 
  ...
  {
      output_data[i] = std::min(std::max(input_data[i] + 3.0f, 0.0f), 6.0f) / 6.0f;
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("hsigmoid", &hsigmoid_cpu, "HSigmoid activation function (CPU)");
}       
