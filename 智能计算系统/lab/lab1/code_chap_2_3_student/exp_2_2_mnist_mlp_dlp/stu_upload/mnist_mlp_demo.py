...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784, 
                    hidden_dims=None, out_classes=10): # 建立网络结构
        ...
        # creating layers
        # TODO：使用 pycnml 建立五层全连接神经网络结构
        self.net.setInputShape(batch_size, input_size, 1, 1) #设置输入参数
        layer_dims = [input_size] + self.hidden_dims + [out_classes]
        self.fc_layer_indices = []
        ...
        for idx in range(len(layer_dims) - 1):
            ...
            input_shape[1] = 1
            input_shape[2] = 1
            input_shape[3] = layer_dims[idx]

            weight_shape = pycnnl.IntVector(4)
            weight_shape[0] = batch_size
            weight_shape[1] = 1
            weight_shape[2] = layer_dims[idx]
            weight_shape[3] = layer_dims[idx + 1]

            output_shape = pycnnl.IntVector(4)
            output_shape[0] = batch_size
            output_shape[1] = 1
            output_shape[2] = 1
            output_shape[3] = layer_dims[idx + 1]

            self.net.createMlpLayer('fc%d' % (idx + 1), input_shape, weight_shape, output_shape)
            self.fc_layer_indices.append(idx * 2)

            if idx < len(layer_dims) - 2:
                self.net.createReLuLayer('relu%d' % (idx + 1))
    ...
    def load_model(self, param_dir):   # 加载参数
        # TODO：使用pycnml接口分别为五层全连接层加载参数
        print('Loading parameters from file ' + param_dir)
        
        params = np.load(param_dir,allow_pickle=True,encoding="latin1").item()
        for idx, layer_idx in enumerate(self.fc_layer_indices, start=1):
            weight = params['w%d' % idx].flatten().astype(np.float64)
            bias = params['b%d' % idx].flatten().astype(np.float64)
            self.net.loadParams(layer_idx, weight, bias)
