...
DEVICE="${3:-cuda}"
GPU_IDS="${4:-0,1,2,3}"
OUTPUT_DIR="${5:-runs/poolnet_uncertainty_route_boundary_r18/main}"

ACTION="$(python scripts/run_guard.py \
  --output-dir "${OUTPUT_DIR}" \
  --expect model=poolnet_uncertainty_route_boundary_r18 \
...
if [ "${ACTION}" = "skip" ]; then
  ...
  python train.py \
    ...
    --model poolnet_uncertainty_route_boundary_r18 \
    --pretrained \
    --image-size 352 \
    --batch-size 16 \
    --epochs 200 \
