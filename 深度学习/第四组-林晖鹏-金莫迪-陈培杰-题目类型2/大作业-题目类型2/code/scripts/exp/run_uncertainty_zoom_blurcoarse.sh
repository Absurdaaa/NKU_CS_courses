...
DEVICE="${3:-cuda}"
GPU_IDS="${4:-0,1}"
OUTPUT_DIR="${5:-runs/uncertainty_zoom/blurcoarse}"
ZOOM_TOPK="${6:-5}"

ACTION="$(python scripts/run_guard.py \
  --output-dir "${OUTPUT_DIR}" \
  --expect model=resnet18_uncertainty_zoom_blurcoarse \
  ...
  --expect zoom_delta_weight=0.0005 \
...
if [ "${ACTION}" = "skip" ]; then
  ...
  python train.py \
    ...
    --model resnet18_uncertainty_zoom_blurcoarse \
    --pretrained \
    --image-size 352 \
    --batch-size 16 \
    --epochs 200 \
