...
set -euo pipefail
# =========================================================================
#  PoolNet-PSG-R18 (PoolNet + morphological completeness loss)
#  Same as PoolNet, but loss = BCE + PSGLoss (closing-based missing-region penalty)
#  Adam lr=5e-5, epochs=100
# =========================================================================
...
train() {
    ...
    CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" python train.py --data-root "$DATA_ROOT" --split-file "$SPLIT_FILE" \
        ...
        --device cuda --gpu-ids "$CUDA_DEVICE" --scheduler step --min-lr 5e-6 --grad-clip 1.0 \
