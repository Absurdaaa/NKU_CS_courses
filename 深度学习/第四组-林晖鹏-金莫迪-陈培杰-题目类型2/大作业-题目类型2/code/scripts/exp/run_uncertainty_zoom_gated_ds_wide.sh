...
DEVICE="${3:-cuda}"
GPU_IDS="${4:-0,1}"
OUTPUT_DIR="${5:-runs/uncertainty_zoom/k5_gated_ds_wide}"
ZOOM_TOPK="${6:-5}"
ZOOM_VIS_EVERY="${7:-5}"
ZOOM_VIS_COUNT="${8:-2}"
...
ACTION="$(python scripts/run_guard.py \
  ...
  --expect model=resnet18_uncertainty_zoom_gated_ds_wide \
  ...
  --expect zoom_patch_weight=0.5 \
  --expect zoom_delta_weight=0.001 \
  --expect zoom_side_weight=0.2 \
  --expect zoom_loss=hybrid \
  --expect disable_zoom_coarse_channel=false)"
...
if [ "${ACTION}" = "skip" ]; then
  ...
  python train.py \
    ...
    --model resnet18_uncertainty_zoom_gated_ds_wide \
    --pretrained \
    --image-size 352 \
    --batch-size 16 \
    --epochs 200 \
    ...
    --zoom-patch-weight 0.5 \
    --zoom-delta-weight 0.001 \
    --zoom-side-weight 0.2 \
    --zoom-loss hybrid \
    --zoom-vis-every "${ZOOM_VIS_EVERY}" \
    --zoom-vis-count "${ZOOM_VIS_COUNT}" \
    --output-dir "${OUTPUT_DIR}"
fi
